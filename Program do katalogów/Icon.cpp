...
( FPTR_##procname ) GetProcAddress ( GetModuleHandleA (  _TEXT( #dllname)), #procname);
...
HICON GoFindAnIcon(LPCSTR path,INT wielkosc)
{
	HICON icon = 0;

	//not using our parent's icon
	if(_tcsicmp("parent", path))
	{
		icon = (HICON)LoadImageA(0, path, IMAGE_ICON, GetSystemMetrics(SM_CXICON), GetSystemMetrics(SM_CYICON), LR_LOADFROMFILE | LR_LOADMAP3DCOLORS);
		if(!icon)
		{
			//Try something else
			...
			if (wielkosc != 0)
			ExtractIconExA(cleanpath, index, &icon , 0, 1);
			else ExtractIconExA(cleanpath, index, 0 ,&icon , 1);
		}
	}
	else
	{
